...
VERSION=v1.0.3
ISO_VOL=SPLNAS
ISO_FILE=splnas-installation-offline-$VERSION.iso
NAS_VERSION="$VERSION-offline-stable"
DATE=`date +%F/%T`
export LC_ALL=C

function set_version(){
  buildiso_mount_rootfs
  sudo chroot $(pwd)/squashfs-root /bin/bash -c "echo > /etc/nas_version;echo VERSION=$NAS_VERSION >> /etc/nas_version;echo DATE=$DATE >> /etc/nas_version"
...
#  sudo chroot $(pwd)/squashfs-root /bin/bash -c "[ -f /etc/proxychains.conf ] && rm /etc/proxychains.conf"
  ...
  sudo chroot $(pwd)/squashfs-root /bin/bash -c "apt update; apt install -y mysql-client pv curl htop net-tools vim byobu proxychains dnsutils mc jq git sshpass python3-setuptools ntpdate gdebi xz-utils"
  ...
  sudo cp -rf /news/packages/Casic-Client-DockerDeploy $(pwd)/squashfs-root/usr/local/src/
  sudo cp -rf /news/packages/first_init $(pwd)/squashfs-root/usr/local/src/
  sudo cp -rf /news/packages/server_config_check $(pwd)/squashfs-root/usr/local/src/
  sudo cp -rf /news/packages/docker_image/nas-base-image.tar.gz $(pwd)/squashfs-root/usr/local/src/
  sudo cp -rf /news/packages/docker_image/base-image.tar.gz $(pwd)/squashfs-root/usr/local/src/
  sudo cp -rf /news/code/first_init_offline.sh $(pwd)/squashfs-root/usr/
...
function buildiso_chroot_exec_install_zabbix()
{
  echo "chroot exec install zabbix ..."
  sudo cp -rf /news/packages/zabbix/requirements.txt  $(pwd)/squashfs-root/mnt/
  sudo cp -rf /news/packages/package  $(pwd)/squashfs-root/mnt/
  sudo cp -rf /news/packages/zabbix/zabbix-0.0.3-py3-none-any.whl  $(pwd)/squashfs-root/mnt/
  sudo cp -rf /news/packages/zabbix/scripts  $(pwd)/squashfs-root/tmp/
  sudo cp -rf /news/packages/zabbix/docker_status.conf  $(pwd)/squashfs-root/tmp/
  sudo cp -rf /news/system-config/splfw   $(pwd)/squashfs-root/tmp/
...
#  sudo chroot $(pwd)/squashfs-root /bin/bash -c "cd /mnt; apt install python3-pip -y; pip3 --default-timeout=600 install --upgrade setuptools; pip3 --default-timeout=600 install  -r  requirements.txt; pip3 install  zabbix-0.0.3-py3-none-any.whl"
  ...
  sudo cp -rf /news/packages/zabbix/zabbix.yml  $(pwd)/squashfs-root/etc/zabbix/
}

function buildiso_chroot_exec_install_lcdproc()
{
  echo "chroot exec install lcdproc ..."
  sudo cp -rf /news/packages/lcdproc  $(pwd)/squashfs-root/mnt/
  sudo cp -rf /news/packages/lcd_script  $(pwd)/squashfs-root/usr/local/src/
  buildiso_mount_rootfs
...
function buildiso_install_mon_package()
{
  ...
  cp -R -f /news/packages/spl-mon-serv squashfs-root/mon-temp 
...
function buildiso_chroot_exec_upgrade()
{
  ...
  sudo chroot $(pwd)/squashfs-root /bin/bash -c "apt-get full-upgrade -y"
